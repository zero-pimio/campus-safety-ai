...
当前边界：本地 MP4 已跑通，摄像头/RTSP 尚未接入；ByteTrack、OpenRemote/MQTT、RKNN/昇腾板卡、截图/短录像和校园数据精度也未验证。这些属于后续门禁，不能把本骨架当成已部署系统。
...
训练输出包含逐 epoch 的 `metrics.jsonl`、验证 F1 最优的 `best.pt`、ONNX 模型和输入预处理元数据。当前公开数据只能建立基线；最终阈值与验收必须使用按摄像头或日期隔离的校园视频。
